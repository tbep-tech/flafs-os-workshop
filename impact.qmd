...
Alternative formats are specified the same way. 
...
You can also render all formats at once using the `quarto` package or from the terminal tab.  The path to your file you want to render will differ depending on where it is in your working directory. 

```r
quarto::quarto_render('data/quartoex.qmd')
```

In the terminal (bottom left pane of RStudio), execute the following to render all formats.  This requires the separate installation of Quarto described in the [setup](https://tbep-tech.github.io/flafs-os-workshop/setup.html#install-quarto).

```{.bash filename="Terminal"}
quarto render data/quartoex.qmd
```

Rendering a file to PDF uses LaTeX and you'll need to install tinytex before you can use this option.  

```{.bash filename="Terminal"}
quarto install tool tinytex
```

There are also several options you can include in the YAML to control the formatting of the output.  Some of the options apply to all format types, whereas others are specific to each.  Here's an example building out these options further.  
...
There are many other options available for each output format, as well as other format types.  View the full list here: [https://quarto.org/docs/output-formats/all-formats.html](https://quarto.org/docs/output-formats/all-formats.html){target="_blank"}

### Citations and References

It's entirely possible to create reports and manuscripts with Quarto, not to mention much easier since your analysis, data, and writing can all be done in the same place. All of the above examples apply to "authoring", e.g., adding figures and tables.  Here, we'll discuss a few other useful features of Quarto for authoring documents. 
...
You can of course do this using the source editor, but it's slightly easier using the visual editor.  If we switch to visual mode, you can type a forward-slash to view a menu of items to insert in the document. Just start typing text to search items to insert.

::: {.callout-tip}
The [Insert Anything](https://quarto.org/docs/visual-editor/options.html#insert-anything){target="_blank"} tool in the visual editor mode is useful to... insert anything! Just execute `/` at the beginning of a line or `Ctrl/Cmd + /` after some test.
:::
...
The text citation will look like this, where `@` is the tag used to reference the citation.
...
The `@` citation syntax also allows different options for how the citation is displayed in the text (full explanation [here](https://quarto.org/docs/authoring/footnotes-and-citations.html){target="_blank"}.  For example, omitting the brackets does the following:
