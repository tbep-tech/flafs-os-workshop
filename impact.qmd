...
```markdown
![](https://www.ncwildlife.org/Portals/0/Learning/images/Species/Fish/Black-Crappie-Duane-Raver.jpg)
```
...
Rendering a Quarto file to an HTML, PDF, or Word document is as simple as adding the appropriate option to the YAML.  You can select the format when you create a new Quarto file just by selecting the appropriate option: 
...
You can use the dropdown menu to render each file format one at a time.

![](img/renderoption.PNG)

You can also render all formats at once using the `quarto` package or from the terminal tab.  The path to your file will differ depending on where it is in your working directory. 
...
There are also several options you can include in the YAML to control the formatting of the output.  Some of the options can apply to all format types, whereas others are specific to each.  Here's an example building out these options further.  
...
In the above example, there are two new options that apply globally to the HTML, Word, and PDF outputs.  Specifically, we've indicated that we'd like a table of contents (`toc: true`) and that the sections should be numbered (`number-sections: true`).

We'ave also added some specific options to the HTML and PDF output.  For the HTML output, we've indicated that we want the code chunks to be folded (i.e., toggle between seen and not seen).  For the PDFoutput, we've slightly changed the geometry of the margins. 

Here's what the HTML output would look like: 

![](img/formatoptions.PNG){.border}

There are many other options aviable, as well as other format types.  View the full list here: [https://quarto.org/docs/output-formats/all-formats.html](https://quarto.org/docs/output-formats/all-formats.html){target="_blank"}

### Authoring

It's entirely possible to create reports and manuscripts with Quarto, not to mention much easier since your analysis, data, and writing can all be done in the same place.  
